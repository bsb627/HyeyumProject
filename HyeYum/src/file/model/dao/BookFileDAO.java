...
//			pstmt.setInt(5, review.getNo());
	...
	public FileData selectFileOneInfo(Connection conn, int showNo) {
		PreparedStatement pstmt = null;
		ResultSet rset = null;
	...
	private int getShareNo(Connection conn, BookShare share) {
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		String query = "SELECT SHARE_NO FROM BOOK_SHARE WHERE REGION = ? AND TITLE= ? AND CONTENTS = ? AND ENROLL_DATE = ? AND USER_ID = ? ORDER BY ENROLL_DATE DESC";
		int shareNo = 0;
		...
		try {
			...
			pstmt.setDate(4, share.getEnrollDate());
			pstmt.setString(5, share.getUserId());
			rset = pstmt.executeQuery();
			if(rset.next()) {
	...
	public ArrayList<FileData> selectFileListShare(Connection conn) {
	...
	public FileData selectFileOneShare(Connection conn, int showNo) {
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		String query = "SELECT * FROM SHOW_FILE WHERE REVIEW_NO=?";
		FileData fileData = null;
